...
// Determines the winner based on moves.
...
func rpsGame(c *websocket.Conn) {
	...
	// Game begins when both players joined.
	...
	for {
		...
		if ok && action == "move" {
			...
			// Proceed once both moves have been made.
			if len(game.Moves) == 2 {
...
func main() {
	...
	// WebSocket endpoint.
	app.Get("/ws/:game_code", websocket.New(rpsGame))
